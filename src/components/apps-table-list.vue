<template>
    <f7-list class="apps-table-list" no-chevron no-hairlines :style="styleString">
      
        <f7-list-item v-for="app in apps"
          :link="`/app/${app.id}`"
          :key="app.id"
          :route-props="{backText}"
        >
          ...
          <img loading="lazy" class="apps-table-list-image" slot="media" :src="app.icon" :alt="app.title" />
