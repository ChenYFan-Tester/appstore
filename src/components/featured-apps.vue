<template>
    <div class="block featured-apps">
        <div class="featured-app" v-for="app in apps" :key="app.id">
          <f7-link no-link-class :href="`/app/${app.id}`" :route-props="{ backText }">
            ...
            <img loading="lazy" class="featured-app-image" :alt="app.title" :src="useIcon ? app.icon : app.screenshots[0]" />
          </f7-link>
        </div>
    </div>
</template>
