<template>
    <f7-card expandable class="appstore-card">
      <f7-card-content :padding="false">
        <div :class="['appstore-card-header', `appstore-card-header-text-${titlePosition}`]">
          <img :src="image" :alt="imageAlt" />
          <div class="appstore-card-header-text">
            <div class="appstore-card-subtitle" :style="{color: subtitleColor}">{{subtitle}}</div>
            <div class="appstore-card-title" :style="titleStyle" v-html="title"></div>
          </div>
          
          <AppsTableList v-if="app" :styleString="{color: appColor}" :apps="[app]" />
...
<script>
import AppsTableList from './apps-table-list.vue';
